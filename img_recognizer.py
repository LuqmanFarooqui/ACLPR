...
#    clear()
    ...
    for j in range(len(result["plates"])):
      ...
      label = [str(j+1), result["plates"][j]["text"]]
      for i in range(3):
        if i<2:
          k[i] = list(map(int, k[i][:3]+[k[i][-1]]))
          img = cv.rectangle(img,(k[i][0],k[i][1]),(k[i][2],k[i][3]),clr[i],3)
          x1, y1 = k[i][0], k[i][1]
        else:
          x1, y1 = (10, 50)
          label.append("Number Plates found: "+str(len(result["plates"])))
        (w, h), _ = cv.getTextSize(label[i], cv.FONT_HERSHEY_SIMPLEX, 0.6, 2)    
        img = cv.rectangle(img, (x1, y1 - 20), (x1 + w, y1), clr[2], -1)
        img = cv.putText(img, label[i], (x1, y1-5), cv.FONT_HERSHEY_SIMPLEX, 0.6, clr[3], 2)

    impath = str(args.image)
    impath = impath[:-4]+"_out"+impath[-4:]
...
#     input("\nPress Enter to exit...\n") 
    ...
    time1 = tic - toc
    # cv.imshow('image',img)
    print("Time taken for output : " + str(time1) + " seconds")
